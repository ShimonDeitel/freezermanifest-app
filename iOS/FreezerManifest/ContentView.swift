...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Freezer Manifest")
            .toolbar {
...
struct EntryFormView: View {
    ...
    init(entry: Entry?, onSave: @escaping (Entry) -> Void) {
        _fieldValue = State(initialValue: entry?.itemName ?? "")
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                Form {
                    Section("Item name") {
                        TextField("Item name", text: $fieldValue)
                            .accessibilityIdentifier("entryFieldInput")
                    }
                    Section("Frozen") {
                        DatePicker("Date", selection: $date, displayedComponents: .date)
                    }
                    Section("Freezer Zone") {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let entry = Entry(
                            id: entryID ?? UUID(),
                            itemName: fieldValue.isEmpty ? "Untitled" : fieldValue,
                            date: date,
                            note: note
                        )
                        onSave(entry)
